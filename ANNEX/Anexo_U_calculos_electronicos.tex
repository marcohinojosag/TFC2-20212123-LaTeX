...
\begin{subfigure}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{ANNEX/Fig/AU/Diseno_rele1.drawio.pdf}
...
\begin{subfigure}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{ANNEX/Fig/AU/Diseno_Rele2.drawio.pdf}
...
Dado que la bobina del relé demanda aproximadamente \(I_1 \approx 70~\mathrm{mA}\) y que el indicador LED se diseña con una corriente baja, se asume una corriente de diseño que incluya un margen adicional a la suma de estas corrientes, definida en la Ecuación~\ref{eq:ic_diseno}:
...
\(I_{\text{LED}}\in[2~\mathrm{mA},\,5~\mathrm{mA}]\) y, para un LED típico, \(V_{F,\text{LED}}\in[1.8~\mathrm{V},\,2.2~\mathrm{V}]\).
De acuerdo con \eqref{eq:rled}, el rango de resistencias se obtiene evaluando los casos extremos:

\begin{align}
R_{\text{LED,min}}
&\approx \frac{5~\mathrm{V}-1.8~\mathrm{V}-0.1~\mathrm{V}}{5~\mathrm{mA}}
\approx 620~\Omega,
\label{eq:rled_min}
\\
R_{\text{LED,max}}
&\approx \frac{5~\mathrm{V}-2.2~\mathrm{V}-0.2~\mathrm{V}}{2~\mathrm{mA}}
\approx 1300~\Omega.
\label{eq:rled_max}
\end{align}
...
% (Opcional, si deseas justificar tu elección práctica)
% En la implementación también se verificó que un valor mayor, \(R_{\text{LED}}=4.7~\mathrm{k}\Omega\),
% produce un encendido visible con menor consumo; no obstante, dicha selección reduce la corriente del LED
% por debajo del rango de diseño y puede depender del tipo de LED y de las condiciones de iluminación.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.78\linewidth]{ANNEX/Fig/AU/Vbe_SAT.drawio.pdf}
    \caption{Tensión base-emisor del 2N3904 en saturación, \(V_{BE(sat)}\), en función de \(I_C\).}
    \label{fig:2n3904_vcesat}
\end{figure}

Aplicando la Ley de Voltajes de Kirchhoff en el lazo de excitación de base (referido a 5~V) se obtiene la Ecuación~\ref{eq:kvl_rb}:
...
Adoptando rangos típicos de diseño \(V_{CE(sat),\text{PC817}}\in[0.1~\mathrm{V},0.2~\mathrm{V}]\) y \(V_{BE(sat)}\in[0.85~\mathrm{V},0.95~\mathrm{V}]\), junto con el rango de \(I_B\) de la Ecuación~\ref{eq:ib_rango}, al sustituir en la Ecuación~\ref{eq:rb} se obtiene:

\begin{align}
R_{B,\min} &\approx \frac{5~\mathrm{V}-0.2~\mathrm{V}-0.95~\mathrm{V}}{3.3~\mathrm{mA}} \approx 1166.67~\Omega, \\
R_{B,\max} &\approx \frac{5~\mathrm{V}-0.1~\mathrm{V}-0.85~\mathrm{V}}{3.33~\mathrm{mA}} \approx 1227.27~\Omega.
\label{eq:rb_rango}
\end{align}
...
\qquad
I_{R3}\approx\frac{5~\mathrm{V}}{10~\mathrm{k}\Omega}=0.5~\mathrm{mA}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.78\linewidth]{ANNEX/Fig/AU/V_forward.pdf}
...
A partir de la Figura~\ref{fig:pc817_vf} se considera \(V_{F,\text{PC817}}\in[1.2~\mathrm{V},1.4~\mathrm{V}]\). Para una activación robusta del canal se adopta \(I_F\in[8~\mathrm{mA},10~\mathrm{mA}]\). Sustituyendo estos rangos en la Ecuación~\ref{eq:r1}, se obtiene:

\begin{align}
R_{1,\min} &\approx \frac{3.3~\mathrm{V}-1.4~\mathrm{V}}{10~\mathrm{mA}} \approx 190~\Omega, \\
R_{1,\max} &\approx \frac{3.3~\mathrm{V}-1.2~\mathrm{V}}{8~\mathrm{mA}} \approx 262.5~\Omega.
\label{eq:r1_rango}
\end{align}
...
\begin{equation}
R_{\mathrm{div}} > 1~\mathrm{k}\Omega\left(\frac{12}{3.3}-1\right) = 2.64~\mathrm{k}\Omega.
\label{eq:rdiv_num}
\end{equation}
...
Para garantizar que la componente fundamental de la señal \ac{PWM} de \(10~\mathrm{kHz}\) se transmita correctamente y, a su vez, atenuar componentes por encima de dicha banda, se selecciona un filtro con \(f_c \gtrsim 15~\mathrm{kHz}\). De la Ecuación~\ref{eq:fc_rc_pwm} se obtiene el producto \(RC\) requerido:

\begin{align}
RC&=\frac{1}{2\pi f_c}
\approx \frac{1}{2\pi(15\times 10^3)}
\approx 10.6~\mu\mathrm{s}.
\label{eq:rc_pwm_obj}
\end{align}
...
\Rightarrow
f_c=\frac{1}{2\pi(1~\mathrm{k}\Omega)(10~\mathrm{nF})}\approx 15.9~\mathrm{kHz}.
\label{eq:rc_pwm_sel}
\end{align}
...
En esta implementación se selecciona \(R_{\mathrm{sense}}=560~\Omega\) para incorporar márgenes ante variaciones del factor \(K\) y picos de corriente.
...
Para mantener un ancho de banda suficiente para la dinámica de corriente y, a la vez, atenuar componentes por encima de \(15~\mathrm{kHz}\), se selecciona \(f_c \approx 15~\mathrm{kHz}\). Por tanto,
