A continuación se presenta el código inicial utilizado para configurar el sensor TMAG5170A1 y detectar el giro de un imán a partir de la variación de sus polos. El programa establece la comunicación \ac{SPI} con el ESP32, configura los registros necesarios del sensor y calcula el ángulo en el plano \(XY\) y la velocidad angular a partir de la derivada temporal del ángulo.

% =========================================================
% ANEXO: Listing en C++ (pegar en Anexo \ref{an:cod_tmag})
% Requiere en el preámbulo: \usepackage{listings}
% =========================================================

\begin{lstlisting}[
language=C++,
caption={Configuración mínima del TMAG5170A1 y cálculo de ángulo y velocidad angular en el plano \(XY\) mediante \ac{SPI} (\ac{ESP32}).},
label={lst:tmag5170_cfg_xy},
numbers=left,
numberstyle=\tiny,
stepnumber=1,
...
// Deshabilitar CRC mediante TEST_CONFIG (bytes: 0x00 0x04 0x07)
...
// Config: modo de medicion activo + habilitar XYZ + rango ±100 mT
...
static const AngleMode MODE = ANGLE_MODE_XY;
...
void loop() {
  ...
  if (prevUs != 0) {
    ...
    float ddeg = unwrapDeltaDeg(angleDeg, prevAngle);    // deg
