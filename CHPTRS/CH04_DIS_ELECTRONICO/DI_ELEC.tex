...
\section{Selección de componentes electrónicos}
    El \ac{HW} del sistema realizará tres tareas principales: energizar el motor de pruebas, sensar sus variables físicas (voltaje, corriente y velocidad angular) y enviar los datos   dichas variables hacia la interfaz. Por ello, se comenzará realizando la selección de componentes que permitan realizar las tareas.
    ...
    Sensado de corriente
      & Sí, pin CS proporcional a corriente del motor
      & Pines \textit{Sense} para resistores externos (sin ganancia integrada)
      & Sí, pin IS proporcional a corriente del motor \\
    Protecciones
   ...
   \subsection{Selección de sensor de corriente}
    ...
    Para la medición de corriente del motor se aprovecha el sensado integrado del puente H VNH7070BAS, el cual entrega en el pin CS una señal proporcional a la corriente del motor. Esta decisión evita incorporar un sensor de corriente \ac{DC} adicional, reduciendo la complejidad del \ac{HW} y el área requerida en la \ac{PCB}. En la Sección~\ref{sec:diseño_driver} se detalla el circuito de adecuación necesario para que la señal CS sea compatible con el ADC del microcontrolador (\SI{0}{\volt}--\SI{3.3}{\volt}).
    
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.55\linewidth]{CHPTRS/CH04_DIS_ELECTRONICO/Fig_Componentes/DIS_ELEC_06_DriverVNH.jpg}
        \caption{Puente H VNH7070BAS utilizado y salida de sensado de corriente (CS)}
        \label{fig:vnh7070_cs}
        \caption*{\footnotesize\textit{Nota: Figura tomada de \cite{ST_VNH7070BAS}}}
    \end{figure}
    
    En términos generales, la señal del pin CS entrega una corriente proporcional a la corriente del motor, la cual se convierte a voltaje mediante la resistencia \(R_{\text{sense}}\) que se presentará en la sección \ref{sec:diseño_driver}. Considerando un factor de escalamiento efectivo \(K_{\text{eff}}\) obtenido del \textit{datasheet} del \textit{driver}, la corriente del motor puede expresarse como
    \[
    I_{\text{motor}} = \frac{K_{\text{eff}}}{R_{\text{sense}}}\,V_{\text{ADC}} \;\pm\; \Delta I,
    \]
    donde \(\Delta I\) representa un término de corrección (sumado o restado) que se ajustará experimentalmente mediante la comparación con un sensor de corriente de referencia en el Capítulo~\ref{ch:CH07}.


    \subsection{Selección de fuente de alimentación}

        Dado el requisito de conexión directa a red (\SI{220}{\volt}–\SI{60}{\hertz}), se seleccionó una \ac{SMPS} en lugar de una fuente lineal. Las \ac{SMPS} ofrecen mayor eficiencia y densidad de potencia, reduciendo su volumen al prescindir del transformador de \SI{60}{\hertz} propio de las topologías lineales; estas características favorecen la integración del sistema en un chasis compacto (véase \cite{fswitchignfuentelineal}). 
        \begin{figure}[H]
            \centering
            \begin{tabularx}{\linewidth}{>{\centering\arraybackslash}X}
                % Fila 1: imagen
...
\section{Diseño de \ac{PCB}}
    ...
    VBAT recibe el voltaje nominal del motor. Se coloca un condensador cerámico de \SI{100}{\nano\farad} lo más próximo posible a VBAT--AGND y, en la entrada, un condensador electrolítico (\SI{47}{\micro\farad}--\SI{220}{\micro\farad}) para amortiguar transitorios. La alimentación lógica VCC (3,3--5\,V) lleva su propio \SI{100}{\nano\farad} junto a los pines del CI.
    ...
    INA, INB, \ac{PWM} y SEL0 se conectan a GPIO de la ESP32 con resistencias \textit{pull-down} de \SI{10}{\kilo\ohm} para evitar estados flotantes al arranque. La señal \ac{PWM} opera típicamente entre 10–20\,kHz, por lo que queda a criterio de diseño elegir la frecuencia de esta señal.
    ...
    Mecánicamente, la placa debe alinearse para mantener el imán coaxial al encapsulado, de modo que el centro del imán coincida con el centro del CI y se conserve una holgura axial estable.
        ...
        \begin{itemize}
          ...
          \item Lectura periódica de ANGLE\_RESULT y calibración de offset de cero.
        \end{itemize}
...
\section{Diseño de las \ac{PCB}s}
    ...
    \begin{figure}[H]
       \centering
       \includegraphics[width=0.6\textwidth]{CHPTRS/CH04_DIS_ELECTRONICO/Fig_Esquematicos/DistribuciónDeComponentes.pdf}
